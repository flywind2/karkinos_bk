...
public class TextSummary {
	...
	private static void writeReadSummary(BufferedWriter bw,
		...
		try {
					...
					if (np.containsKey(key)) {
						...
						if(key.equals("duplicate")){
							...
							try{
								// row 13
								bw.write("duplicate (%) \t");						 				
								bw.write(format(getParcent(np.getProperty(key),np.getProperty("nonduplicate"))) + "%"+"\t");
								bw.write(format(getParcent(tp.getProperty(key),tp.getProperty("nonduplicate"))) + "%"+"\t");
								bw.write("\n");
							}catch(Exception ex){}
						}
			...
			//
			Map<String, ReadsCounter> datamapn = readsSummary.getNormalPerChrom();
			Map<String, ReadsCounter> datamapt = readsSummary.getTumorPerChrom();
